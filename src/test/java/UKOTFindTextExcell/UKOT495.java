...
public class UKOT495 {
  ...
  static WebDriver driver;
    ...
    public static void activityTest() throws Exception {
        ...
        goToPage(TESTING_ROW + n, TESTING_CART_CELL);
        Thread.sleep(2000);
    ...
    public static void findExpectedText(int rowNum, int colNum) throws IOException, InterruptedException{
        ...
        try{
            TEXT=driver.findElement(By.cssSelector("body > app-root > cx-storefront > main > cx-page-layout > cx-page-slot.TokoLeftContent.has-components.ng-star-inserted > cx-cart-details > div > cx-cart-item-list > div > div > cx-cart-item > div > div.cart-item__details > div.pzn-redemption.pzn-green"));
                      System.out.println(TEXT.getText());
            fis = new FileInputStream(FILE);
            workbook = new XSSFWorkbook(fis);
            sheet = workbook.getSheetAt(0);
            row = sheet.getRow(rowNum);
            row.createCell(colNum).setCellValue(TEXT.getText());
            ...
            sheet = workbook.getSheetAt(0);
            row = sheet.getRow(rowNum);
            row.createCell(colNum).setCellValue("NA");

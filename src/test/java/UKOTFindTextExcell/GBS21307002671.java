...
public class GBS21307002671 {
    ...
    static String XLS = "/exel/GBS21307002671/data.xlsx";
...
//#A836-slim > div > h2
            ...
            meta.click();
            Thread.sleep(2000);
            String meta_text2 = driver.getCurrentUrl();

            System.out.println("Founud -  "+meta_text);
            System.out.println("Founud -  "+meta_text2);
            fis = new FileInputStream(FILE);
            workbook = new XSSFWorkbook(fis);
            sheet = workbook.getSheetAt(0);
            row = sheet.getRow(rowNum);
            row.createCell(colNum+1).setCellValue(meta_text);
            row.createCell(colNum+2).setCellValue(meta_text2);
